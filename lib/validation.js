...
const oldCaseSchema = z.object({
  ...
  // The paper register has none of these, so an old case is entered without
  // them; editing it later can add any of the new-case details.
  maritalStatus: z.enum(["Single", "Married", "Divorced", "Widowed"]).optional().or(z.literal("")),
  qualification: optionalName,
  email: z.string().trim().email("Enter a valid email address").optional().or(z.literal("")),
  assistiveDevices: optionalName,
  sourceOfIncome: optionalName,
