...
export function matchesCaseFilters(c, filters) {
  // Any run of digits from the CNIC, typed with or without its dashes.
  const cnicDigits = digitsOnly(filters.cnic);
  if (cnicDigits && !digitsOnly(c.cnic).includes(cnicDigits)) return false;
...
export function hasActiveCaseFilters(filters) {
  return Object.entries(filters).some(([key, v]) =>
    key === "cnic" ? digitsOnly(v) !== "" : v !== "" && v != null
  );
}
